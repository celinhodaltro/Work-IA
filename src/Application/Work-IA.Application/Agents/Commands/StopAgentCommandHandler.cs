...
public sealed class StopAgentCommandHandler : IRequestHandler<StopAgentCommand>
{
    private readonly IAgentRepository _repository;
    private readonly AgentRegistry _agentRegistry;

    public StopAgentCommandHandler(IAgentRepository repository, AgentRegistry agentRegistry)
    {
        _repository = repository;
        _agentRegistry = agentRegistry;
    }

    public async Task Handle(StopAgentCommand request, CancellationToken cancellationToken)
    {
        ...
        if (runtimeAgent is not null)
            await runtimeAgent.StopAsync(cancellationToken);
    }
}
